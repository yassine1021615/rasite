<?php require_once 'partial/header.php'; ?>
      ...
      <main>
        ...
        $count = 0;
            ...
            $sql = "SELECT id, firstname, adverb, surname, email FROM users WHERE deleted = 0";
        ...
        <section class="pages">
          ...
          <?php if($_GET['page'] > 1){ ?> <a href="gebruikers.php?page=<?php echo $_GET['page']-1; ?>"><</a><?php }else{ ?><p><</p><?php } ?>
          <?php $rows = $result->num_rows; while($rows >= 0){ ?>
            <a href="gebruikers.php?page=<?php echo $number ?>"><?php echo $number ?></a>
            <?php $number++; $rows-=8;$loop_count++;} ?>
          <?php if($_GET['page'] <= floor($result->num_rows / 8)){ ?> <a href="gebruikers.php?page=<?php echo $_GET['page']+1; ?>">></a><?php }else{ ?><p>></p><?php  } ?>
        </section>
      </main>
